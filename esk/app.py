...
def get_username():
    env_username = os.environ.get("ESKER_USERNAME", "").strip()
    if env_username:
        return env_username
    ...
    if not username:
        raise ValueError("Username is required to continue.")
...
path_page = r"C:/Users/"+ username + r"/Downloads/esker_merged/"
...
def invoice_download_code():
    ...
    def success_html():
        ...
        def hover(driver, elem_to_hover):
            #elem_to_hover = driver.find_element(By.XPATH, x_path)
            elem_to_hover = pyautogui.moveTo(30,70, duration=2)
            hover = ActionChains(driver).move_to_element(elem_to_hover)
            hover.perform()
        try:
            ...
            pyautogui.moveTo(30,70, duration=1.5) #back arrow previous webpage
            time.sleep(1)
            pyautogui.click(button='left')
        ...
        try:
            ...
            #print(e)
            pyautogui.moveTo(410,420, duration=1.5) #move to HTML2 btn
            pyautogui.click(button='left')
            time.sleep(1.5)
    ...
    def create_zip_file():
        ...
        try:
            ...
            print(e)
            """
        time.sleep(5) #wait for zip file to download
        
        try:
            pyautogui.moveTo(25,70, duration=1.5) #back arrow previous webpage
            time.sleep(1)
    ...
    write_log((f'Process started at {datetime.now().strftime("%Y%m%d %H:%M:%S")}'))
    ...
    invoice_workbook = os.environ.get("ESKER_INVOICE_WORKBOOK")
    invoice_sheet = os.environ.get("ESKER_INVOICE_SHEET", "invoice")

    if invoice_workbook:
        print(f"Loading invoices from workbook '{invoice_workbook}' (sheet '{invoice_sheet}').")
        source_path = invoice_workbook
        source_sheet = invoice_sheet
    else:
        source_path = path_invoice + 'invoice.xlsx'
        source_sheet = 'invoice'

    df_invoice_user_input = pd.read_excel(
        source_path,
    ...
    list_invoice_numbr_original = df_invoice_user_input['invoice'].values.tolist()
    #list_invoice_numbr_original = [item for sublist in list_invoice_numbr_original for item in sublist]
    # convert all elements in list to str
    list_invoice_numbr = [str(elem) for elem in list_invoice_numbr_original]
    # append 3 zeros
    #list_invoice_numbr = [['000' + str(i) for i in sublist] for sublist in list_invoice_numbr]
    #list_invoice_numbr = [item for sublist in list_invoice_numbr for item in sublist]
       
    #list_invoice_numbr = [str(elem) for elem in list_invoice_numbr_original] # convert all elements in list to str
    ...
    for invoice_numbr in list_invoice_numbr:
            ...
            dct_invoice_indices = get_invoice_number_indices(list_invoice_numbr_on_page, list_common_invoice_numbr)
    ...
    for key,value in new_dct_invoice_indices_all.items():
        ...
        doc_num_pages=max_num
        print(f"Maximum number pages: {max_num}")  # Output: Maximum number: 2

        time.sleep(5)
        if doc_num_pages is not None:
            max_num= find_max_number(list_docNum_class_all)
            doc_num_pages = max_num #iterate through all pages, save pdf
            doc_num_pages = max_num
            def process_pages(num_pages):
                """
                Process pages from 1 to num_pages.    
                Args:
                    num_pages (int): Total number of pages to process.
                """
                # Check if num_pages is None or not an integer
                if num_pages is None:
                    #raise ValueError("num_pages cannot be None. Please provide a valid integer.")
                    return    
            ...
            for p in range(1, doc_num_pages+1):
                ...
                invoice_numbr = key
                pyautogui.moveTo(350,570, duration=1.5) #to save multi-page pdf
                #pyautogui.click(button='right')
                ...
                write_log(f'{invoice_numbr} No documents to download at {datetime.now().strftime("%Y%m%d %H:%m")}')
        ...
        write_log(f'Download process completed at {datetime.now().strftime("%Y%m%d %H:%M:%S")}')
        list_invoice_downloaded.append(invoice_numbr)
        
        dir="C:/Users/"+username+"/Downloads/"
        def get_html2_again(dir):
            list_html_file = glob.glob(dir+'*.html')
            try:
                list_latest_html_file = [max(list_html_file, key=os.path.getctime)]
            except ValueError:
                list_latest_html_file = []    
                pyautogui.moveTo(25,70, duration=2.5) #pageback btn
                pyautogui.click(button='left')
                time.sleep(0.5)

                try:             
                    #html_2 = driver.find_element(By.XPATH, '//*[@id="ZipPane_eskCtrlBorder_content"]/div/div[2]/div[2]/div[3]/div[1]/div/span[1]')
                    html_2 = WebDriverWait(driver, 10).until(EC.presence_of_element_located((By.XPATH, '//*[@id="ZipPane_eskCtrlBorder_content"]/div/div[2]/div[2]/div[3]/div[1]/div/span[1]'))
                ...
                html_quit = driver.find_element(By.XPATH, '//*[@id="form-footer"]/div[1]/a/span')
                html_quit.click()
            return list_latest_html_file

        def create_zip_file_again():
            pyautogui.moveTo(20,615, duration=1) #move to small check box (View)
            pyautogui.click(button='left')
            time.sleep(0.5)
            pyautogui.moveTo(160,545, duration=1) #move to Download documents button
            pyautogui.click(button='left')
            time.sleep(3.5) #wait for the page to load
            pyautogui.moveTo(600,415, duration=1.5) #move to small check box (Include audit trail report)
            pyautogui.click(button='left')
            pyautogui.moveTo(250,700, duration=1) #move to Create ZIP file button
            pyautogui.click(button='left')
            time.sleep(3)

            pyautogui.moveTo(620,220, duration=1.5) #move to blank space
            pyautogui.click(button='left')
            time.sleep(0.5)
            pyautogui.hotkey('ctrl', 'shift', 'r')  # refresh page
            time.sleep(2.5) #wait for the page to refresh
            pyautogui.moveTo(620,220, duration=1.5) #move to blank space
            pyautogui.click(button='left')
            time.sleep(0.5)
            pyautogui.hotkey('ctrl', 'shift', 'r')  # refresh page
            ...
            pyautogui.moveTo(25,65, duration=1.5) ##move to back arrow (top left corner)
            pyautogui.click(button='left')
            time.sleep(0.5) #wait to click html2
        ...
        try:
            ...
            try:
                create_zip_file_again() #if lisT_latest_html_file is empty
                html_2 = html_2 = WebDriverWait(driver, 10).until(EC.element_to_be_clickable((By.XPATH, '//*[@id="ZipPane_eskCtrlBorder_content"]/div/div[2]/div[2]/div[3]/div[1]/div/span[1]'))
        ...
        fldr = r"C:/Users/"+username+r"/Downloads/esker_merged/"
        ...
        # write to excel
        fldr_invoice_downloaded = r"C:/Users/"+ username + r"/Downloads/esker_merged/"
        ...
        """
        write_log(f'Process completed at {datetime.now().strftime("%Y%m%d %H:%M:%S")}')
        #done
